...
class QuestionController {
  ...
  deleteQuestion(req, res) {
    ...
    db.query(query2, (error2, res2) => {
        if (error2) { return res.status(400).json({error: 'Something went wrong with the process, Please try again later' });
        } else {
            if (res2.rows.length) {
                ...
                db.query(query5, (error5, res5) => {
                    if (error5) { return res.status(400).json({error: 'Something went wrong with the process, Please try again later'});
                    } else {
                        if(res5.rows.length) {
                        ...
                        } else { return res.status(403).json({error: 'The question you are trying to delete doesn\'t belong to you' });
                        }
                    }
                });
